...
class ActiveBaseModel(models.Model):
    ...
    active = models.SmallIntegerField(verbose_name="状态", default=1, choices=active_choices)

    class Meta:
        abstract = True  # 不会在数据库中生成表
...
class Level(ActiveBaseModel):
    ...
    percent = models.IntegerField(verbose_name="折扣", help_text="填入0-100整数表示百分比，例如90表示90%")
